<template>
  <div class="table-main-box">
    ...
    <!-- 机构展示弹窗 -->
    <component :is="'org'" :handler-row="handlerRow" ref="cmpEl" :flatDistrictOptions="flatDistrictOptions"
      :districtFirstOptions="districtFirstOptions" :disabled="true" :footerSlot="true" :showSelectPerson="false"
      v-model="handlerControlVisible" v-if="handlerControlVisible" :isSelectCase="true"></component>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadPage() {
      ...
      this.$api("earTag/queryListTTargeByPage", params)
        .then((res) => {
          if (res.code === 200) {
            let list = res.data.list || [];
            // 处理每一行数据的机构代码
            const promises = list.map(item => {
              return this.$api("earTag/selectTargetConfigComcode", item.configId)
                .then(res => {
                  if(res.code === 200) {
    ...
    formatRequestParams(map) {
      ...
      return {
        ...
        comcodes: map.comCodeList || [],     // 从机构选择器获取的机构代码数组
        configId: map.configId || '',        // 配置ID
        configStatus: map.configStatus || '',      // 状态
    ...
    handlerChange({ row, sign }) {
      ...
      if (row) {
        this.loading = true;
        this.$api("earTag/queryByIdTTarge", row.configId)
          .then((res) => {
    ...
    handlerRelease({ row }) {
      this.$confirm(`确认发布该规则?`, "提示", {
        ...
        this.loading = true;
        this.$api("earTag/publishOneTTarge", row.configId)
          .then(() => {
    ...
    handlerDelist({ row }) {
      this.$confirm(`确认下架该规则?`, "提示", {
        ...
        this.loading = true;
        this.$api("earTag/offLineTTarge", row.configId)
          .then(() => {
    ...
    handlerDel({ row }) {
      this.$confirm(`确认删除该项?`, "提示", {
        ...
        this.loading = true;
        this.$api("earTag/delTTarge", row.configId)
          .then(() => {
